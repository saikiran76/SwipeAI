...
import { processWithOCR } from '../services/ocrService';
...
const FileUpload = () => {
    ...
    const handleSuccess = (data: ExtractedData) => {
        try {
            // First dispatch customers and products to ensure they exist
    ...
    const handleFileUpload = async (file: File, selectedModel?: 'gemini' | 'ocr') => {
        try {
            ...
            // Process file using selected model
            const fileContent = await readFileContent(file);
            setProcessingStatus(`Processing with ${selectedModel.toUpperCase()}...`);

            let extractedData;
            try {
                extractedData = selectedModel === 'gemini'
                    ? await extractDataFromDocument(fileContent, file.type)
                    : await processWithOCR(fileContent, file.type);
            } catch (error) {
